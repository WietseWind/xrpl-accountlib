...
- `signAndSubmit(tx, wss, account)` (async) can be used to sign a transaction & submit it to the network provided:
  - `Sequence`, `Account`, `LastLedgerSequence`, and `Fee` can be omitted.
  - The second param. takes either an instance of `xrpl-client` or a string containing a WebSocket node endpoint. In case of a string (endpoint) a connection will be created & closed automatically.
