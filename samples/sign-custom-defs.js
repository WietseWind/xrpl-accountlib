...
;(async () => {
  const defsCall = await fetch('https://xahau.network', { method: 'POST', body: '{"method":"server_definitions"}', });
  const defs = (await defsCall.json()).result;
  const definitions = new lib.XrplDefinitions(defs);

  
  console.log("XRPL-AccountLib (with Custom Definitions");
  console.log(definitions.getNativeAsset())
  console.log();
